...
3. モジュールの改造 (line-edit, console)
   ...
   - Windowsの標準入出力のハンドルをキャッシュしないように修正(2017-8-23)
   - Windowsのとき、enqueue-keybufferでLFをバッファに入れないように修正(2017-8-23)  
     (ckw-modで、複数行のテキストをペーストすると、改行が2倍になっていたため)
   - getchのタイムアウト関連の処理修正(2017-8-23)
...
3. mintty 上で、入力がエコーバックされたり、入力が消えたりする。  
   ...
   なった。このため call-with-console をやめて元に戻した。  
   (結局、完全な対策にはなっていない。。。))
...
- 2017-8-23  v2.90 標準モジュールの更新に追従(console.scm, generic.scm, windows.scm)  
  ...
  Windowsのとき、enqueue-keybufferでLFをバッファに入れないように修正  
  getchのタイムアウト関連の処理修正  
  termiosのMSYS対応処理見直し  
  REPLの処理見直し
